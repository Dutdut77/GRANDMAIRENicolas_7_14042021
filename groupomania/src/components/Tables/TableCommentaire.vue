      ...
      <table class="table table-hover table-bordered mt-3" id="tableComment">
        ...
        <tbody>
          <tr v-for="(commentaire, index) in commentaires" :key="index">
            ...
            <td class="trash" @click="Modal(commentaire)"><fa :icon="['fas', 'trash-alt']" /></td>
          </tr>
        </tbody>
      </table>
      <p v-if="commentaires.length > 1">Nombre de commentaires   : {{ commentaires.length }}</p>
...
<div v-else class="tableAdmin" >
    ...
    <transition name="modal">
      <Modal v-if="showModalCommentaire" @close="showModalCommentaire = false">
        ...
        Attention vous êtes sur le point de supprimer le commentaire "{{commentaire.content}}" de {{commentaire.pseudo}} du {{commentaire.date}}.
        Etes-vous sur ?   
        </template>
        <template v-slot:footer>    
          <button class="modal-save-btn" @click="Delete(commentaire.id)">SUPPRIMER</button>
